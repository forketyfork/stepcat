...
export class CodexRunner {
  ...
  async run(options: CodexRunOptions): Promise<{ success: boolean; output: string }> {
    ...
    const result = await new Promise<{ exitCode: number | null; output: string; error?: Error }>((resolve) => {
      ...
      if (timeout > 0) {
        timeoutId = setTimeout(() => {
          child.kill('SIGTERM');
          resolve({ exitCode: null, output: stdoutData, error: new Error('Codex execution timed out') });
        }, timeout);
      }
